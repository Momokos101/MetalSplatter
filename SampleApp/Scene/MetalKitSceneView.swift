...
struct MetalKitSceneView: ViewRepresentable {
    ...
    class Coordinator: NSObject {
        ...
        var magnificationGesture: NSMagnificationGestureRecognizer?
        var rotationGesture: NSRotationGestureRecognizer?
    ...
    private func setupGestures(for view: MTKView, coordinator: Coordinator) {
        ...
        // 放大手势
        let magnificationGesture = NSMagnificationGestureRecognizer(target: coordinator, action: #selector(Coordinator.handleMagnification(_:)))
        view.addGestureRecognizer(magnificationGesture)
        coordinator.magnificationGesture = magnificationGesture
...
extension MetalKitSceneView.Coordinator {
    ...
    @objc func handleMagnification(_ gesture: NSMagnificationGestureRecognizer) {
        ...
        case .began:
            lastScale = renderer.scale
        case .changed:
            let newScale = lastScale * (1.0 + Float(gesture.magnification))
            // 限制缩放范围
            let clampedScale = max(0.1, min(5.0, newScale))
            renderer.updateScale(clampedScale)
        case .ended, .cancelled, .failed:
